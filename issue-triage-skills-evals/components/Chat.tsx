...
// Starter prompts chosen to show precise, on-demand retrieval: juggling two
// issues (it loads two runbooks), the ambiguous word "timeouts" (it should load
// the ONE right runbook, not blend the ~8 that mention it), a deep step-by-step
// that exercises the SLA-first + Refs rules, and a refund over $50 (policy R4 —
// which now holds because it's no longer buried). Watch the activity log and the
// (small) context meter.
        ...
        <div className="mx-auto flex max-w-3xl flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            ...
            <p className="text-xs text-zinc-500">
              Chat triage that loads only the runbook it needs.
            </p>
          </div>
...
function MessageRow({
  ...
  // openai reasoningSummary option on the agent). gpt-5* only exposes a SUMMARY of
  ...
  // The agent's tool calls → the live activity timeline: which runbook it loaded
  // (skill) and any incident-archive grep (bash). This is the demo's payoff —
  // you can see it pull in ONLY the relevant runbook, not all 18.
  const activity: Activity[] = message.parts.filter(isToolUIPart).map((part) => {
    const hasInput =
  ...
  // Bridge the gap after the tools finish and before the answer text streams.
  if (active && !text && activity.length > 0) {
    activity.push({ id: "__composing__", kind: "composing", status: "running" });
  }

  // Show the thinking panel while streaming with no answer or tool call yet
  // (even before the first reasoning token), or whenever reasoning exists.
  const showPanel = !!reasoning || (active && !text && activity.length === 0);
  const hasAside = activity.length > 0 || showPanel;

  return (
    <li className="flex justify-start">
      <div className="flex max-w-[85%] flex-col gap-2">
        {activity.length > 0 && <AgentActivity activity={activity} />}
  ...
  return (
    <div className="rounded-2xl border border-dashed border-zinc-200 px-6 py-10 text-center">
      ...
      <p className="mx-auto mt-1.5 max-w-md text-sm leading-relaxed text-zinc-500">
        This copilot keeps a thin prompt and pulls in the right runbook on demand.
        Ask it something and watch the activity log show exactly which one it
        loaded — while the context meter stays small.
      </p>
